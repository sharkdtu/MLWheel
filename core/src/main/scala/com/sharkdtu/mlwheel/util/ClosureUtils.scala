...
/**
 * A Util for cleaning closure
 */
private[mlwheel] object ClosureUtils extends Logging {
  ...
  private def ensureSerializable(func: AnyRef) {
    try {
      PSContext.get.closureSerializer.newInstance().serialize(func)
    } catch {
      case ex: Exception => throw new PSException("Task not serializable", ex)
    }
  }
...
private[mlwheel] class ReturnStatementFinder extends ClassVisitor(ASM4) {
      ...
      new MethodVisitor(ASM4) {
        override def visitTypeInsn(op: Int, tp: String) {
          if (op == NEW && tp.contains("scala/runtime/NonLocalReturnControl")) {
            throw new PSException("Return statements aren't allowed in Spark closures")
          }
        }
      }
    } else {
...
private[mlwheel] class FieldAccessFinder(output: Map[Class[_], Set[String]])
...
private[mlwheel] class InnerClosureFinder(output: Set[Class[_]]) extends ClassVisitor(ASM4) {
